...
/*************************************************************************
 *  Author: Dr Akalanka Hewawasam
 *  IIT Student no - 2015254
 *  UOW ID - w1608470
 * *************************************************************************/
...
public class Astar {
	...
	public static double getEucVal(double x1, double x2, double y1, double y2) {
		double xDiff = x1 - x2;
	...
	public static void AStar(int metrix) {
...
//					}
	 ...
	 * coordinates
	 */
	public static void test(int tCase, int x, int y, int si, int sj, int ei, int ej, int[][] blocked, int metrics) {
		System.out.println("\n\nTest Case #" + tCase);
		// Reset
		grid = new Cell[x][y];
		...
		if (closed[endI][endJ]) {
			...
			while (current.parent != null) {
				...
				totLength = totLength + getEucVal(current.i, current.parent.i, current.j, current.parent.j);
				}

				System.out.print(" -> " + current.parent);
				StdDraw.circle(current.j, N - current.i - 1, 0.5);
				current = current.parent;

			}
			System.out.println();
			System.out.println("length  " + totLength);
			System.out.println();
		} else
